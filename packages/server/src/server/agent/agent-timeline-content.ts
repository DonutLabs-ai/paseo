...
function copyStringPrefix(value: string, maxLength: number): string {
  return Buffer.from(value.slice(0, maxLength), "utf16le").toString("utf16le");
}
...
function limitFailedShellError(item: AgentTimelineItem): AgentTimelineItem {
  ...
  return {
    ...
    error: {
      ...item.error,
      content: copyStringPrefix(item.error.content, TOOL_CALL_CONTENT_MAX_LENGTH),
    },
  };
}
...
function limitPlainText(item: AgentTimelineItem): AgentTimelineItem {
  ...
  return {
    ...
    detail: {
      ...item.detail,
      text: copyStringPrefix(item.detail.text, TOOL_CALL_CONTENT_MAX_LENGTH),
    },
  };
}
...
function truncateUtf8(text: string, maxBytes: number, suffix = TIMELINE_TRUNCATION_NOTICE): string {
  ...
  return `${copyStringPrefix(text, low)}${suffix}`;
}
    ...
    case "user_message":
      return {
        ...
        ...(item.clientMessageId
          ...
          : {}),
        ...(item.scheduleId ? { scheduleId: truncateIdentifier(item.scheduleId) } : {}),
        ...(item.scheduleRunId ? { scheduleRunId: truncateIdentifier(item.scheduleRunId) } : {}),
      };
...
export function limitAgentTimelineItemContent(item: AgentTimelineItem): AgentTimelineItem {
  ...
  item = {
    ...
    detail: {
      ...item.detail,
      output: copyStringPrefix(item.detail.output, TOOL_CALL_CONTENT_MAX_LENGTH),
    },
  };
  return limitAgentTimelineItemSize(item);
}
