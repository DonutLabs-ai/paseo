...
function launchPackagedGui({ executablePath, env, launchArgs = [] }) {
  const stdout = [];
  const stderr = [];
...
async function reserveDistinctLocalTcpPort(excludedPorts) {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const port = await reserveLocalTcpPort();
    if (!excludedPorts.includes(port)) return port;
  }
  throw new Error("Failed to reserve a distinct local TCP port");
}
...
async function smokePackagedDesktopApp({
  ...
  const firstCdpPort = await reserveDistinctLocalTcpPort([daemonPort]);
  ...
  const launches = [];
  let currentLaunch = launchPackagedGui({ executablePath, env: firstEnv, launchArgs });
  ...
  try {
    ...
    const secondCdpPort = await reserveDistinctLocalTcpPort([daemonPort, firstCdpPort]);
    ...
    currentLaunch = launchPackagedGui({ executablePath, env: secondEnv, launchArgs });
    launches.push(currentLaunch);
    ...
    if (expectedSandbox !== undefined) {
      ...
      await openSmokeWorkspace({ appPath, env: firstEnv, page, daemonHome });
    }
