...
export class AgentStoreProjection {
  ...
  commitCached(agents: Map<string, Agent>): Map<string, Agent> {
    ...
    for (const agent of merged.values()) {
      this.publishActivity(agent);
      this.wakeWorkspaceForScheduleRun(agent);
    }
  ...
  accept(agent: Agent): Agent {
    ...
    this.wakeWorkspaceForScheduleRun(accepted);
    return accepted;
  }
  ...
  replaceFetched(entries: FetchAgentsEntry[]): Map<string, Agent> {
    ...
    for (const agent of agents.values()) this.wakeWorkspaceForScheduleRun(agent);
  ...
  private wakeWorkspaceForScheduleRun(agent: Agent): void {
    ...
    useCockpitSnoozeStore
      .getState()
      .wakeForScheduleRun(`${this.serverId}:${agent.workspaceId}`, scheduleRunStartedAt);
  }
}
