...
describe("workspace-layout-store helpers", () => {
  it("seeds Files, Changes, and References in the default Explorer sidebar", () => {
...
describe("workspace-layout-store migrations", () => {
  ...
  it("adds References once to a version 2 Explorer without changing its selected tab", async () => {
    const currentLayout = createWorkspaceLayoutWithExplorerSidebar();
    const versionTwoLayout = {
      ...currentLayout,
      root: removeTabFromTree(currentLayout.root, "references"),
    };
    ...
    await AsyncStorage.setItem(
      ...
      JSON.stringify({
        state: {
          layoutByWorkspace: { [workspaceKey]: versionTwoLayout },
    ...
    const explorer = findPaneById(
      migrated.getState().layoutByWorkspace[workspaceKey].root,
      "explorer",
    );
    expect(explorer?.tabIds).toEqual(["files", "changes_tree", "references"]);
    expect(explorer?.focusedTabId).toBe("changes_tree");
    ...
    expect(reloadedExplorer?.tabIds).toEqual(["files", "changes_tree", "references"]);
    expect(reloadedExplorer?.focusedTabId).toBe("changes_tree");
  });
  ...
  it("does not replace Explorer again when the migrated layout reloads", async () => {
    await persistVersionOneLayout("v0.6");
    const migrated = createWorkspaceLayoutStore(createDeterministicWorkspaceLayoutIds());
    await migrated.persist.rehydrate();
...
describe("workspace-layout-store actions", () => {
  ...
  it.each(["explorer", "pane_generated_explorer"])(
    ...
    async (explorerId) => {
      ...
      expect(findPaneById(layout.root, explorerId)).toMatchObject({
        id: explorerId,
        hidden: true,
        tabIds: ["files", "draft_saved", "references"],
        focusedTabId: "draft_saved",
      });
  ...
  it("persists and rehydrates independent Changes state through validated storage", async () => {
    ...
    await vi.waitFor(async () => {
      ...
      expect(collectTabIds(root)).toEqual([first, second, "files", "changes_tree", "references"]);
    });
  ...
  it("hides and shows Explorer without changing its default tabs or split sizes", () => {
    ...
    expect(findPaneById(layout.root, paneId)).toMatchObject({
      hidden: true,
      tabIds: ["files", "changes_tree", "references"],
    });
  ...
  it("focusTab reveals an Explorer tab without moving workspace focus", () => {
    ...
    expect(findPaneById(layout.root, paneId)).toMatchObject({
      ...
      tabIds: ["files", "changes_tree", "references", targetTabId],
    });
    expect(findPaneById(layout.root, paneId)?.hidden).toBeUndefined();
  ...
  it("closing Files leaves Changes available in Explorer", () => {
    ...
    expect(hiddenPane?.tabIds).toEqual(["changes_tree", "references"]);
    expect(
  ...
  it("closing Files keeps Changes on screen without removing the final ordinary pane", () => {
    ...
    expect(finalPane?.tabIds).toEqual(["changes_tree", "references"]);
    expect(
  ...
  it("leaves the default tabs in Explorer when content is claimed by main", () => {
    ...
    expect(sidePane?.tabIds).toEqual(["files", "changes_tree", "references"]);
    expect(findPaneById(layout.root, paneId)?.hidden).toBeUndefined();
  });
