...
Accepting a new turn clears unread `finished` or `error` attention from the previous turn at the
same lifecycle boundary. Permission attention remains until the permission is explicitly resolved.
