...
| `pendingAutomaticRetry` | `{ reason: "usage_limit", retryAt: string }?` (nullable) | Durable Codex usage-limit continuation deadline; cleared on manual continuation, archive, or explicit close.                                                                                                                                                                                                                                                                        |
...
| `promptMessageId` | `string?`                              | Stable prompt identity for navigable agent runs |
...
| `title`                        | `string \| null`                                             | User-set workspace note/name override layered over `displayName`. The editor caps it at 200 single-line characters. Null means "use `displayName`".                                           |
